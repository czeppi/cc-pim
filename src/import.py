...
def main():
    revisions = [create_first_json_revision(datetime.datetime.today())]
...
class RowsComparer:
    ...
    def compare(self):
        diff = RowsDiff()
        row_map1 = self.data1.create_row_map()
        row_map2 = self.data2.create_row_map()
...
class RowsDiff:
    ...
    def change_row(self, old_row, new_row):
        self.changed_rows.append((old_row, new_row))
...
def create_first_json_revision(date_time):
...
def create_json_revision(date_time, changes):
    return {
        "date": date_time.strftime('%Y-%m-%d_%H:%M:%S'),
        "user": "Christian Czepluch",
        "action": "import",
        "changes": changes,
    }
